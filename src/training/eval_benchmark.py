...
def load_model():
    ckpt = torch.load(
        os.path.join(MODEL_DIR, "cnn_trimmed_checkpoint.pt"), weights_only=False
    )
...
def score_session(filepath, model, mean, std):
    ...
    for start in range(0, len(abstract) - WINDOW_SIZE + 1, 5):
        ...
        cat = torch.tensor(
            [[TOOL_TO_IDX.get(s["tool"], 6) for s in window]], dtype=torch.long
        )
        cont_raw = []
        for s in window:
            all_vals = [
                (
                    float(s.get(f, 0))
                    if not isinstance(s.get(f, 0), bool)
                    else (1.0 if s.get(f, 0) else 0.0)
                )
...
def main():
    ...
    if not results_dir or not os.path.isdir(results_dir):
        print(
            "Set BENCHMARK_RESULTS_DIR to the directory containing off_1/, off_2/, heldout_off_1/"
        )
        sys.exit(1)
    ...
    for run in ["off_1", "off_2", "heldout_off_1"]:
        ...
        for fname in sorted(os.listdir(run_dir)):
            ...
            session_file = find_session_file(
                sid, task_slug, claude_projects, worktree_base
            )
            if not session_file:
                if not paths_mode:
            ...
            if paths_mode:
                ...
                print(
                    f"  {task:25s} {n_steps:3d}st  max={max_s:.3f} ({fired:2d}/{len(scores)}){tag}"
                )
                summary[f"{run}/{task}"] = {
                    "max": max_s,
                    "fired": fired,
