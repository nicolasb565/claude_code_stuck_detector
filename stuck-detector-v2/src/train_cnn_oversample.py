...
def main():
    ...
    # ---- Benchmark on LogReg sessions ----
    print(f"\n=== Benchmark: LogReg tasks (off_1, off_2, heldout_off_1) ===")
    ...
    def make_window_input(window):
        ...
        return cat, cont, torch.tensor([wf], dtype=torch.float32)
    ...
    def score_session(filepath):
        ...
        for start in range(0, len(abstract) - WS + 1, 5):
            ...
            cat, cont, wf = make_window_input(window)
            with torch.no_grad():
                s = torch.sigmoid(model(cat, cont, wf)).item()
            scores.append(s)
    ...
    for run in ['off_1', 'off_2', 'heldout_off_1']:
        ...
        for f in sorted(os.listdir(run_dir)):
            ...
            scores, n_steps = score_session(session_file)
            if scores is None:
                print(f"  {task:25s} {n_steps:3d} steps (short)")
                continue

            max_s = max(scores)
            fired = sum(1 for s in scores if s >= t)
    ...
    # Save benchmark summary for README generation
