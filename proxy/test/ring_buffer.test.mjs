...
describe('RingBuffer', () => {
  test('initial buildInput is zero-padded except for curr', () => {
    ...
    assert.equal(inp.length, 48) // 8*(1+5)
    for (let i = 0; i < 8; i++) assert.equal(inp[i], 1, `curr[${i}]`)
    for (let i = 8; i < 48; i++) assert.equal(inp[i], 0, `history[${i}]`)
  })
  ...
  test('T-2 slot is correct after two pushes', () => {
    const ring = new RingBuffer(5, 8)
    ring.push(new Float32Array(8).fill(0.1))
    ring.push(new Float32Array(8).fill(0.2))
  ...
  test('oldest entry evicted after N+1 pushes', () => {
    const ring = new RingBuffer(3, 4) // depth=3, dim=4 → input length = 4*4 = 16
    // Push 4 steps: step0(value=1) gets evicted by step3(value=4)
    for (let v = 1; v <= 4; v++) ring.push(new Float32Array(4).fill(v))
  ...
  test('buildInput does not mutate the ring', () => {
    const ring = new RingBuffer(5, 8)
    ring.push(new Float32Array(8).fill(0.5))
